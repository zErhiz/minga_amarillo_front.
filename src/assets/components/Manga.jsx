...
const Manga = () => {
  const {title2, categories2}= useSelector(store=>store.inputs)
...
const [reload, setReload]=useState(false)
  ...
  useEffect(
    ()=>
    {
      ...
      axios(apiUrl+`mangas?title=${title.current.value}&category_id=${values.join(',')}&page=${page}&limit=6&order=1`).then(res=>setMangas(res.data.response)).catch(err=>(console.log(err)))},
...
function capture_text(){
  ...
  dispatch(inputs_filter({
    ...
    categories2:Object.values(category_id.current)
  ...
  return (
   ...
   <div className='bg-bgmanga bg-no-repeat bg-cover bg-[39%] sm:bg-cover   h-[100vh]  w-[100%] sm:h-[60vh]'>
      <div className='flex flex-col justify-center items-center p-52'>
       ...
       <label htmlFor=""></label>
       <input className='text-2xl w-[100%] p-1 rounded-md'defaultValue={title2} type="search" name='Find Your Manga here' placeholder='Find your manga here'ref={title}  onKeyUp={capture_text} />
 </div>
</div>
...
<form ref={category_id}>
  ...
  <label htmlFor={category._id}key={category._id}style={{backgroundColor:category.hover, padding:'0.3rem', borderRadius:'20px'}}>
    {category.name}
    <input 
    defaultChecked={categories.includes(category_id)}
    name="category_id"
    onChange={checkbox}
    type='checkbox'
    value={category._id}
 ...
 <div className='bg-white flex flex-wrap p-9 flex justify-center items-center sm:w-[60vw] w-[100vw]  '>
  ...
  <div className='flex gap-96 p-9 w-screen justify-between'>
   {page != 1&& <button className='border border-black p-4 w-[20%] rounded-md' onClick={prev}>PREV</button>}
    <button className='border border-black p-4 w-[20%] rounded-md rounded-md' onClick={next}>NEXT</button>
  </div>
 </div>
